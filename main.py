...
def cleanup_session_files(session_id: str):
    ...
    if session_id in session_files:
        for task_id in session_files[session_id]:
            ...
            # Remove from tasks if still there
            if task_id in tasks:
                del tasks[task_id]
        ...
        logger.info(f"Cleaned up session {session_id} with {len(session_files.get(session_id, []))} files")
...
async def download_video(task_id: str, url: str, quality: AudioQuality, start_time: int = None, end_time: int = None):
    ...
    try:
        ...
        # More reliable download options
        ydl_opts = {
            'outtmpl': output_path,
...
async def get_video_info(url: str = Query(..., description="YouTube video URL")):
    ...
    try:
        ydl_opts = {'no_warnings': True, 'noplaylist': True}
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
